...
# **Final Summary Table (for Part 4)**

| Method       | Purpose                                       | Example                                |
| ------------ | --------------------------------------------- | -------------------------------------- |
| `concat()`   | Combine DataFrames vertically or horizontally | `pd.concat([df1, df2], axis=0)`        |
| `merge()`    | SQL-style join                                | `df1.merge(df2, on="ID", how="inner")` |
| `pivot()`    | Reshape long → wide                           | `df.pivot(index, columns, values)`     |
| `crosstab()` | Count combinations of values                  | `pd.crosstab(df["A"], df["B"])`        |
