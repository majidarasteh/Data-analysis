...
|                              | `index`                | Shows range of row indices                 | `df.index`                                         | `RangeIndex(0, stop=...)` |
...
|                              | `head(n)`              | Shows top rows (default=5)                 | `df.head(7)`                                       | First 7 rows              |
|                              | `tail(n)`              | Shows bottom rows                          | `df.tail(3)`                                       | Last 3 rows               |
...
|                              | `fillna(df.mean())`    | Fill numerics with mean                    | `df.Price.fillna(df.Price.mean())`                 | NaN → mean                |
|                              | `fillna(df.median())`  | Fill with median                           | `df.EngineV.fillna(df.EngineV.median())`           | NaN → median              |
|                              | `fillna(df.mode()[0])` | Fill categorical with mode                 | `df.Body.fillna(df.Body.mode()[0])`                | Most common value         |
...
| **Column Transformations**   | **`map()`**            | **Transform values using dictionary**      | `df['col'].map(dic)`                               | Values converted          |
| **Sampling / Randomization** | **`sample()`**         | **Randomly select fraction of rows**       | `df.sample(frac=0.5)`                              | Random sample             |
|                              | **`sample(frac=1)`**   | **Shuffle full DataFrame**                 | `df.sample(frac=1)`                                | Random row order          |
| **Saving Changes**           | **Reassignment**       | **Save result of transformed DataFrame**   | `df = df.sample(frac=0.2)`                         | Saves new DataFrame       |
| **Index Operations**         | **`reset_index()`**    | **Reset index after shuffle or filtering** | `df.reset_index(drop=True)`                        | New clean index           |
