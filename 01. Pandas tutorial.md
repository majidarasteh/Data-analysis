...
|                             | `df[['col1','col2']]`  | Select multiple columns               | `df[['Brand','Model']]`                            | DataFrame                  |
...
|                             | `fillna()`             | Fill NaN after adding shorter Series  | `df['Seats'].fillna(2)`                            | NaN → 2                    |
|                             | Assign Python list     | Add column using a list               | `df['Fuel'] = [7.5,6.8,8.0]`                       | Error if length mismatched |
|                             | Column operations      | Create new column using others        | `df['Price_EUR'] = df['Price'] * 0.85`             | New column                 |
|                             | String combining       | Combine text columns                  | `df['Car Name'] = df['Brand'] + ' ' + df['Model']` | e.g., *BMW 320*            |
|                             | Assign same value      | Broadcast a constant value            | `df['Wheels'] = 4`                                 | All rows = 4               |
