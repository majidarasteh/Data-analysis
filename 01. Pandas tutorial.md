...
|                              | MultiIndex             | Create hierarchical index (rows/cols)   | `df.set_index(['Brand','Model'])`                  | Multi-level index        |
|                              | MultiIndex Select      | Select using multi-level index          | `df.loc[('BMW','320i'), :]`                        | Row for BMW 320i         |
|                              | MultiIndex From Arrays | Build MultiIndex manually               | `pd.MultiIndex.from_arrays([A, B])`                | MultiIndex object        |
