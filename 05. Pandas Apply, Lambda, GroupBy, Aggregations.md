...
# **Summary Table — Apply, Lambda, GroupBy, Aggregations**

| **Method / Concept**     | **Description**                                    | **Example**                                                    | **Output**                   |
| ------------------------ | -------------------------------------------------- | -------------------------------------------------------------- | ---------------------------- |
| **`apply()`**            | Applies a function to an entire column or row      | `df["Price"].apply(lambda x: x*2)`                             | Column values doubled        |
| **`apply(axis=1)`**      | Applies a function row-wise                        | `df.apply(lambda r: r["A"] + r["B"], axis=1)`                  | Row-wise sum                 |
| **`lambda`**             | Small anonymous function used inside `apply()`     | `df["A"].apply(lambda x: x+10)`                                | Adds 10 to each value        |
| **`groupby()`**          | Groups data by one or more columns for aggregation | `df.groupby("City")["Sales"]`                                  | A grouped object             |
| **`groupby().sum()`**    | Computes sum for each group                        | `df.groupby("City")["Sales"].sum()`                            | Total sales per city         |
| **`groupby().mean()`**   | Computes mean per group                            | `df.groupby("Department")["Salary"].mean()`                    | Average salary               |
| **`groupby().agg()`**    | Multiple aggregations at once                      | `df.groupby("City")["Sales"].agg(["min","max","mean"])`        | Summary stats table          |
| **`agg()` on DataFrame** | Apply aggregations to multiple columns             | `df.agg({"A":"sum", "B":"mean"})`                              | One-row summary              |
| **Custom Aggregation**   | User-defined functions in agg                      | `df.agg({"A": lambda x: x.max()-x.min()})`                     | Range of A                   |
| **`transform()`**        | Returns same-size output after group operation     | `df.groupby("City")["Sales"].transform("mean")`                | Mean repeated for each row   |
| **`filter()`**           | Keep groups that match a condition                 | `df.groupby("City").filter(lambda g: g["Sales"].mean() > 500)` | Keeps only qualifying groups |
