...
| **Topic**                           | **What It Is**                                                | **Key Features**                                                                                                         | **Important Functions**                                             |
| ----------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------- |
| **1. Stackplot**                    | An area plot where multiple categories are stacked over time. | • Layers stack vertically<br>• Shows cumulative totals<br>• Uses separate colors for each category                       | `plt.stackplot(x, A, B, C)`                                         |
| **2. Subplots (`plt.subplots`)**    | A grid of multiple plots inside one figure.                   | • Organized in rows/columns<br>• Each subplot has its own axes<br>• Can share x/y axes<br>• Supports global figure title | `fig, ax = plt.subplots(r, c)`<br>`fig.suptitle()`<br>`sharex=True` |
| **3. Twin Axes (`twinx`, `twiny`)** | A second axis sharing x or y but with a different scale.      | • `twinx()` adds right-side y-axis<br>• `twiny()` adds top x-axis<br>• Useful for dual-scale charts                      | `ax2 = ax1.twinx()`<br>`ax2 = ax1.twiny()`                          |
