...
# **Summary Table — Datetime, Encoding, Outliers**

| **Category** | **Method / Concept**                        | **Description**                               | **Example**                                              | **Output**                                 |
| ------------ | ------------------------------------------- | --------------------------------------------- | -------------------------------------------------------- | ------------------------------------------ |
| **Datetime** | `pd.to_datetime()`                          | Converts a column to datetime format          | `df["date"] = pd.to_datetime(df["date"])`                | Datetime column                            |
|              | Accessors (`dt.year`, `dt.month`, `dt.day`) | Extract year, month, day                      | `df["year"] = df["date"].dt.year`                        | Year column                                |
|              | `df["date"].dt.strftime()`                  | Format datetime as string                     | `df["fmt"] = df["date"].dt.strftime("%Y-%m")`            | “2025-01”                                  |
|              | `df["date"].dt.day_name()`                  | Extract weekday name                          | `df["weekday"] = df["date"].dt.day_name()`               | “Monday”                                   |
| **Encoding** | **One-Hot Encoding** (`get_dummies`)        | Converts categorical values to binary columns | `pd.get_dummies(df, columns=["color"])`                  | New columns like `color_red`, `color_blue` |
|              | **Label Encoding**                          | Converts categories into numeric labels       | `df["cat"] = df["cat"].astype("category").cat.codes`     | Integers like 0,1,2                        |
|              | Convert to Categorical Type                 | Reduces memory, improves speed                | `df["col"] = df["col"].astype("category")`               | Categorical column                         |
| **Outliers** | Detect using Z-Score                        | Finds values far from mean                    | `df[np.abs((df["v"]-df["v"].mean())/df["v"].std()) > 3]` | Outlier rows                               |
|              | Detect using IQR (Q1, Q3)                   | Identifies values outside 1.5×IQR             | `Q1=df.v.quantile(.25)` `Q3=df.v.quantile(.75)`          | Used for filtering                         |
|              | IQR Filtering                               | Remove outliers                               | `df[(df.v>=Q1-1.5*IQR) & (df.v<=Q3+1.5*IQR)]`            | Cleaned data                               |
|              | **Capping (Winsorizing)**                   | Replace extremes with boundaries              | `df["cap"] = df["v"].clip(lower, upper)`                 | Outliers capped                            |
